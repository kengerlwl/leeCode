    ...
    def pondSizes(self, land):
        ...
        def dfs(x, y, count=0):
            ...
            for i in range(len(dx)):
                ...
                tmpi=x+dx[i]
                tmpj=y+dy[i]
                if tmpi >= 0 and tmpi < high and tmpj >=0 and tmpj < width and land[tmpi][tmpj] == 0 and not flag[tmpi][tmpj]:
                    a = dfs(tmpi, tmpj, m)
                    m = max(m, a)
            return m
        ...
        for i in range(high):
            for j in range(width):
                if land[i][j] == 0 and not flag[i][j]:
                    ans = dfs(i, j)
                    if ans !=0:
                        res.append(ans)
